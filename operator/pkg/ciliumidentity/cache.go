...
// AssignCIDForPod updates the pod to CID map, increments the CID usage and
// returns the CID name of previously set CID and its usage count after
// decrementing the CID usage.
func (c *CIDUsageInPods) AssignCIDForPod(podName, cidName string) (string, int) {
	c.mu.Lock()
	defer c.mu.Unlock()
