...
func (c *Controller) processNextNSQueueItem() bool {
	...
	err := c.reconciler.reconcileNS(key)
	c.handleNSErr(err, item)

	return true
}

func (c *Controller) handleNSErr(err error, key interface{}) {
	if err == nil {
	...
	if c.nsQueue.NumRequeues(key) < maxProcessRetries {
