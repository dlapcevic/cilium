...
func (c *Controller) processNextCIDQueueItem() bool {
	...
	err := c.reconciler.reconcileCID(key)
	c.handleCIDErr(err, item)

	return true
}

func (c *Controller) handleCIDErr(err error, key interface{}) {
	if err == nil {
		c.cidQueue.Forget(key)
		return
	}
	...
	if c.cidQueue.NumRequeues(key) < maxProcessRetries {
		c.cidQueue.AddRateLimited(key)
		return
	}

	// Drop the CES from queue, we maxed out retries.
	log.WithError(err).WithFields(logrus.Fields{
		logfields.CIDName: key,
