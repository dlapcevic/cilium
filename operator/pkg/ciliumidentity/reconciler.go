...
func (r *reconciler) calcDesiredStateOnStartup() error {
	...
	// }
	r.syncCESsOnStartup()
	errList := r.syncPodsOnStartup()
	if len(errList) > 0 {
		// Print errors and return first error.
		return errList[0]
	}
...
func (r *reconciler) reconcileCID(cidResourceKey resource.Key) error {
	//log.Infof("reconcileCID '%s'", cidResourceKey.String())
...
func (r *reconciler) createCID(cidName string, cidKey *key.GlobalIdentity) error {
	...
	log.WithField(logfields.CIDName, cidName).Info("Creating a Cilium Identity")
...
func (r *reconciler) updateCID(cid *cilium_api_v2.CiliumIdentity, cidKey *key.GlobalIdentity) error {
	...
	log.WithField(logfields.CIDName, cid.Name).Info("Creating a Cilium Identity")
...
func (r *reconciler) cleanUpCID(cidName string) {
	//log.Infof("Sending CID '%s' for clean up", cidName)
...
func (r *reconciler) cidIsUsedInCES(cidName string) bool {
	...
	//log.Infof("cidIsUsedInCES - cidName: '%s', count: %d", cidName, cidUsageCount)
...
func (r *reconciler) allocateCID(k8sLabels map[string]string) (string, error) {
	...
	storeCIDs, err := r.cidStore.ByIndex(ByKeyIndex, cidKey.GetKey())
	if err != nil {
		return "", err
	}
	...
	if len(storeCIDs) > 0 {
		return r.handleSameUntrackedCIDs(storeCIDs)
	}
...
func (r *reconciler) handleSameUntrackedCIDs(storeCIDs []*v2.CiliumIdentity) (string, error) {
	if len(storeCIDs) == 0 {
...
func (r *reconciler) updateAllPodsInANS(namespace string) error {
	...
	podList, err := r.podStore.(cache.Indexer).ByIndex(cache.NamespaceIndex, namespace)
	if err != nil {
		return err
	}
